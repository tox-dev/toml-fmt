...
_DISTRIBUTION: Final[str] = "pyproject-fmt"
_MODULE: Final[str] = "pyproject_fmt"
...
def test_vendor_into_wheel_ships_only_python_sources(wheel: Path) -> None:
    with ZipFile(wheel) as archive:
        vendored = {name for name in archive.namelist() if name.startswith("pyproject_fmt/_vendor/")}
    assert vendored == {
...
def test_vendor_into_wheel_requires_dist_in_header(wheel: Path) -> None:
    with ZipFile(wheel) as archive:
        metadata = archive.read("pyproject_fmt-0.dist-info/METADATA").decode()
...
def test_vendor_into_wheel_spells_the_entry_point_import_vendored(wheel: Path) -> None:
    with ZipFile(wheel) as archive:
        assert archive.read("pyproject_fmt/__main__.py") == b"import pyproject_fmt._vendor.toml_fmt_common\n"
...
def test_link_common_into_wheel_points_at_the_source_tree(editable: Path, tmp_path: Path) -> None:
    with ZipFile(editable) as archive:
        assert (
            archive.read("pyproject_fmt_toml_fmt_common.pth").decode()
            == f"{tmp_path / 'package/toml-fmt-common/src'}\n"
        )
