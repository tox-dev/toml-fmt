...
#[test]
fn test_setuptools_packages_find_include_and_exclude_sorted() {
    let start = indoc::indoc! {r#"
    [tool.setuptools.packages.find]
    where = ["src"]
    include = ["zeta*", "alpha*"]
    exclude = ["zeta.tests*", "alpha.tests*"]
    "#};
    insta::assert_snapshot!(evaluate_long(start), @r#"
    [tool.setuptools.packages.find]
    where = [ "src" ]
    include = [ "alpha*", "zeta*" ]
    exclude = [ "alpha.tests*", "zeta.tests*" ]
    "#);
}

#[test]
fn test_setuptools_package_data_values_sorted_per_key() {
    let start = indoc::indoc! {r#"
    [tool.setuptools.package-data]
    zeta = ["z.txt", "a.txt"]
    "*" = ["star2.txt", "star1.txt"]
...
/// setuptools installs each of these in the order they are listed, so which of two files sharing a
/// name ends up installed is what the order says.
#[test]
fn test_setuptools_file_lists_keep_their_order() {
    let start = indoc::indoc! {r#"
    [tool.setuptools]
    script-files = ["z/run", "a/run"]
    data-files.share = ["z/value.txt", "a/value.txt"]
    "#};
    let result = evaluate(start);
...
/// The same list written out under its own header says what the dotted key said, so it is left in
/// the same order.
#[test]
fn test_setuptools_data_files_written_out_keep_their_order() {
    let start = indoc::indoc! {r#"
    [tool.setuptools.data-files]
    share = ["z/value.txt", "a/value.txt"]
    "#};
...
/// Written out under a header of its own, the `*` catch-all leads and the packages follow by name.
/// The catch-all is not one of the names that sort, so it adds nothing to that order.
#[test]
fn test_setuptools_long_format_package_data_leads_with_the_catch_all() {
    let start = indoc::indoc! {r#"
    [tool.setuptools.package-data]
    pkg = ["*.txt"]
    "*" = ["*.cfg"]
