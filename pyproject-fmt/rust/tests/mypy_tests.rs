...
#[test]
fn test_mypy_overrides_that_is_not_an_array() {
    let start = indoc::indoc! {r#"
    [tool.mypy]
    overrides = "none"
    "#};
...
#[test]
fn test_mypy_overrides_member_that_is_not_a_table() {
    let start = indoc::indoc! {r#"
    [tool.mypy]
    overrides = [ "none" ]
    "#};
...
/// An override folded into its parent is written as a table inside an array, and it is the same
/// override whichever way the file holds it.
#[test]
fn test_mypy_overrides_folded_into_an_array_keep_their_key_order() {
    let start = indoc::indoc! {r#"
    [[tool.mypy.overrides]]
    strict = true
    module = ["z", "a"]
    ...
    insta::assert_snapshot!(evaluate(start), @r#"
    [tool.mypy]
    overrides = [ { module = [ "a", "z" ], strict = true } ]
    "#);
}
