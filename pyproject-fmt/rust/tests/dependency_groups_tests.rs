...
#[test]
fn test_format_dependency_groups_multiple_include_groups_keep_their_place() {
    let start = indoc! {r#"
    [dependency-groups]
    test = ['a>=1']
    docs = ['b>=1']
...
#[test]
fn test_dependency_groups_include_groups_keep_their_place() {
    let start = indoc! {r#"
        [dependency-groups]
        all = [
          {include-group = "zebra"},
...
/// A member that names no requirement has nothing to sort by, so the list keeps the order the file
/// wrote it in rather than having the unreadable one carried somewhere else.
#[test]
fn test_format_dependency_groups_holding_a_member_it_cannot_read() {
    let start = indoc! {r#"
    [dependency-groups]
    test = [ "b", [ "nested" ], "a" ]
    "#};
...
#[test]
fn test_format_dependency_groups_keeps_a_requirement_it_cannot_read() {
    let start = indoc! {r#"
    [dependency-groups]
    test = ["good >= 1.0.0", "!! not a requirement !!"]
    "#};
    insta::assert_snapshot!(format_dependency_groups_helper(start, false), @r#"
    [dependency-groups]
    test = [ "!! not a requirement !!", "good>=1" ]
    "#);
}

/// An include puts the group it names where it is written, so it stays there and only the
/// requirements between two of them sort.
#[test]
fn test_dependency_groups_sort_only_between_the_includes() {
    let start = indoc::indoc! {r#"
    [dependency-groups]
    all = ["c<1", {include-group = "test"}, "z>1", "a>1", {include-group = "docs"}, "d>1"]
    "#};
    let result = format_dependency_groups_helper(start, false);

    insta::assert_snapshot!(result, @r#"
    [dependency-groups]
    all = [ "c<1", { include-group = "test" }, "a>1", "z>1", { include-group = "docs" }, "d>1" ]
    "#);
}

/// A group names a list of requirements, and a value that is not one holds nothing the rule reads.
#[test]
fn test_dependency_groups_group_that_is_not_an_array() {
    let start = indoc::indoc! {r#"
    [dependency-groups]
    dev = "not-an-array"
    "#};
