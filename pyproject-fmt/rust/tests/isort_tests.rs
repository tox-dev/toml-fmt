...
/// isort appends each forced-separate group to the sections in the order they are listed, so that
/// order is what the imports come out in.
#[test]
fn test_isort_forced_separate_keeps_its_order() {
    let start = indoc::indoc! {r#"
    [tool.isort]
    forced_separate = ["vendor_z", "vendor_a"]
    known_first_party = ["zeta", "alpha"]
    "#};
    let result = evaluate(start);

    insta::assert_snapshot!(result, @r#"
    [tool.isort]
    known_first_party = [ "alpha", "zeta" ]
    forced_separate = [ "vendor_z", "vendor_a" ]
    "#);
}
