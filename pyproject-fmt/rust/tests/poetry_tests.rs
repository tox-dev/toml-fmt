...
#[test]
fn test_poetry_group_key_without_an_inner_segment_is_left_alone() {
    let start = indoc::indoc! {r#"
    [tool.poetry]
    group.dev = "b"
    "#};
...
/// A group name the file quoted because it holds a dot is one segment.
#[test]
fn test_poetry_group_name_holding_a_dot_is_still_ordered() {
    let start = indoc::indoc! {r#"
    [tool.poetry.group."a.b"]
    include-groups = ["zeta", "alpha"]
    "#};
    let result = evaluate_long(start);
    assert!(result.parse::<toml::Table>().is_ok(), "{result}");
    insta::assert_snapshot!(result, @r#"
    [tool.poetry.group."a.b"]
    include-groups = [ "alpha", "zeta" ]
    "#);
}

/// A dotted group name folded into `[tool.poetry]` is still the one name the file gave it, so its
/// keys keep the poetry order rather than falling back to alphabetical.
#[test]
fn test_poetry_collapsed_group_name_holding_a_dot_keeps_its_order() {
    let start = indoc::indoc! {r#"
    [tool.poetry.group."a.b".dependencies]
    zeta = "1"
    python = "3.12"
    alpha = "1"
    ...
    insta::assert_snapshot!(result, @r#"
    [tool.poetry]
    group."a.b".optional = true
    group."a.b".dependencies.python = "3.12"
    group."a.b".dependencies.alpha = "1"
    group."a.b".dependencies.zeta = "1"
    "#);
}

/// Every rule reads the key's own segments, so a group the file quoted keeps them all.
#[test]
fn test_poetry_collapsed_quoted_group_arrays_sort() {
    let start = indoc::indoc! {r#"
    [tool.poetry]
    group."a.b".include-groups = ["zeta", "alpha"]
    group."a.b".dependencies.foo.extras = ["zeta", "alpha"]
    "#};
    let result = evaluate(start);
    assert!(result.parse::<toml::Table>().is_ok(), "{result}");
    insta::assert_snapshot!(result, @r#"
    [tool.poetry]
    group."a.b".include-groups = [ "alpha", "zeta" ]
    group."a.b".dependencies.foo.extras = [ "alpha", "zeta" ]
    "#);
}

#[test]
fn test_poetry_expanded_quoted_group_dependencies_order() {
    let start = indoc::indoc! {r#"
    [tool.poetry.group."a.b".dependencies]
    zeta = "1"
    python = "3.12"
...
/// A source folded into its parent is written as a table inside an array, and it is the same source
/// whichever way the file holds it.
#[test]
fn test_poetry_source_folded_into_an_array_keeps_its_key_order() {
    let start = indoc::indoc! {r#"
    [[tool.poetry.source]]
    ...
    insta::assert_snapshot!(evaluate(start), @r#"
    [tool.poetry]
    source = [ { name = "held", url = "https://example.com" } ]
    "#);
}
